/*
Modified to keep compatibility to adafruit implementation
Bugfixed blue channel behaviour with Aldi/ICLight strip
Renamed to allow independed updating of orginal adafruit library,
because newer version of adafruit#s lib have issues with the 
Aldi/IClight HW.
...
/*
  Modified to work with TM1829 digital RGB stripes (ALDI Süd Digital RGB Steifen)
  Changed high/low in 16Mhz / 800kHz section
  Runs ONLY @ 800kHz
*/
...
Aldi_NeoPixel::Aldi_NeoPixel(uint16_t n, uint8_t p, uint8_t t) {
  numBytes = n * 3;
...
void Aldi_NeoPixel::setPixelColor(
 uint16_t n, uint8_t r, uint8_t g, uint8_t b) {
  if(n < numLEDs) {
    if(b==255) {b--;} // aldi led strip crashes on blue=255
     
    uint8_t *p = &pixels[n * 3];
    if((type & NEO_COLMASK) == NEO_BRG)       { *p++ = b; *p++ = r; *p = g; }
    else if ((type & NEO_COLMASK) == NEO_GRB) { *p++ = g; *p++ = r; *p = b; }
    else                                      { *p++ = r; *p++ = g; *p = b; }
  }
}

// Set pixel color from 'packed' 32-bit RGB color:
void Aldi_NeoPixel::setPixelColor(uint16_t n, uint32_t c) {
  if(n < numLEDs) {
...
uint32_t Aldi_NeoPixel::Color(uint8_t r, uint8_t g, uint8_t b) {
  return ((uint32_t)r << 16) | ((uint32_t)g <<  8) | b;
}

// Query color from previously-set pixel (returns packed 32-bit RGB value)
uint32_t Aldi_NeoPixel::getPixelColor(uint16_t n) {
  ...
  if(n < numLEDs) {
    ...
    if ((type & NEO_COLMASK) == NEO_GRB) {
    ...
    else if ((type & NEO_COLMASK) == NEO_RGB) {
      return 
      ...
      return 
             ((uint32_t)(pixels[ofs    ]) <<  8) |
             ((uint32_t)(pixels[ofs + 1])      ) |
             ((uint32_t)(pixels[ofs + 2]) << 16);
    }
  }
...
uint16_t Aldi_NeoPixel::numPixels(void) {
  return numLEDs;
}
